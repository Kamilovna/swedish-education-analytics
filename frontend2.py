...
st.subheader(f"Population and granted places by county, Sweden, {year}")

fig.update_traces(marker=dict(color="#E3120B", opacity=0.75, line=dict(width=0)))
...
fig.update_xaxes(title="Population", showgrid=True, gridcolor="#e6e6e6", zeroline=False)
...
st.subheader(f"Granted study places per 1,000 inhabitats by county, Sweden, {year}")
st.markdown(
...
fig_map.update_traces(
    hovertemplate="<b>%{location}</b><br>Places per 1,000: %{z:.2f}<extra></extra>",
...
st.subheader(f"Top 5 educational area by granted places in {year}.")
st.caption("Source: Myndigheten för yrkeshögskolan (MYH)")

resp = requests.get(f"{API_URL}/statistics/top_education_area", params={"year": year})
...
fig.update_traces(marker_color="#e3120B")
